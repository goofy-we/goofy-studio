...
export function Canvas() {
  ...
  const [longPressSelectionActive, setLongPressSelectionActive] = useState(false);
  ...
  const rightClickPanGestureRef = useRef<{
    active: boolean;
    pointerId: number;
    startClientX: number;
    startClientY: number;
  ...
  const longPressSelectRef = useRef<{
    active: boolean;
    pointerId: number;
    startClientX: number;
    startClientY: number;
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      if (event.button === 2) {
        const viewport = reactFlowInstance.getViewport();
        rightClickPanGestureRef.current = {
          active: true,
          pointerId: event.pointerId,
          startClientX: event.clientX,
          startClientY: event.clientY,
      ...
      if (event.button === 0) {
        ...
        longPressTimerRef.current = setTimeout(() => {
          ...
          longPressSelectRef.current = {
            active: true,
            pointerId: event.pointerId,
            startClientX: event.clientX,
            startClientY: event.clientY,
          };
          setLongPressSelectionActive(true);
          const selectionPane = wrapperElement.querySelector('.react-flow__pane');
          if (selectionPane) {
            const pointerDownEvent = new PointerEvent('pointerdown', {
              ...
              pointerId: event.pointerId + 1000,
              pointerType: 'mouse',
              button: 0,
              buttons: 1,
            });
            selectionPane.dispatchEvent(pointerDownEvent);
          }
        }, longPressDelay);
      }
    };
    ...
    const handlePointerMove = (event: PointerEvent) => {
      const rcpGesture = rightClickPanGestureRef.current;
      if (rcpGesture && rcpGesture.active && event.pointerId === rcpGesture.pointerId) {
        const deltaX = event.clientX - rcpGesture.startClientX;
        const deltaY = event.clientY - rcpGesture.startClientY;

        if (!rcpGesture.moved && Math.hypot(deltaX, deltaY) >= 4) {
          rcpGesture.moved = true;
        }
        if (!rcpGesture.moved) {
          return;
        }
        ...
        reactFlowInstance.setViewport(
          {
            x: rcpGesture.startViewportX + deltaX,
            y: rcpGesture.startViewportY + deltaY,
            zoom: rcpGesture.zoom,
          },
          { duration: 0 }
        );
        return;
      ...
      if (event.button === 0 && longPressTimerRef.current && longPressStartPosRef.current) {
        const deltaX = event.clientX - longPressStartPosRef.current.x;
        const deltaY = event.clientY - longPressStartPosRef.current.y;
    ...
    const handlePointerUp = (event: PointerEvent) => {
      if (event.button === 2) {
        const gesture = rightClickPanGestureRef.current;
        if (gesture && event.pointerId === gesture.pointerId) {
          rightClickPanGestureRef.current = null;
          if (gesture.moved) {
            const viewport = reactFlowInstance.getViewport();
            setViewportState(viewport);
            const project = getCurrentProject();
      ...
      if (event.button === 0) {
        ...
        longPressStartPosRef.current = null;
        longPressSelectRef.current = null;
        setLongPressSelectionActive(false);
      }
    };
    ...
    wrapperElement.addEventListener('contextmenu', handleContextMenu);
    wrapperElement.addEventListener('pointerdown', handlePointerDown, true);
    window.addEventListener('pointermove', handlePointerMove, true);
    window.addEventListener('pointerup', handlePointerUp, true);
    window.addEventListener('pointercancel', handlePointerCancel, true);
    ...
    wrapperElement.addEventListener('dblclick', handleNativeDoubleClick);

    return () => {
      wrapperElement.removeEventListener('contextmenu', handleContextMenu);
      wrapperElement.removeEventListener('pointerdown', handlePointerDown, true);
      window.removeEventListener('pointermove', handlePointerMove, true);
      window.removeEventListener('pointerup', handlePointerUp, true);
      window.removeEventListener('pointercancel', handlePointerCancel, true);
      wrapperElement.removeEventListener('dblclick', handleNativeDoubleClick);
    };
  }, [
    cancelPendingViewportPersist,
    getCurrentProject,
